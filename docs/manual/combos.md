...
## 1. Combos
Click the **Combos** tab at the top and one of the available numbers below it. By default, the number of configurable combos is calculated from the EEPROM size on your keyboard's microcontroller. This limit [can be adjusted]({% link manual/advanced/custom-build.md %}#adjusting-feature-limits) at compile time (not the GUI) when building firmware.

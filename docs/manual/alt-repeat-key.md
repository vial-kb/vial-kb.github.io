...
Click the **Alt Repeat Key** tab at the top of the window and one of the available numbers below it to edit a configuration rule. By default, the number of configurable Alt Repeat Key entries is calculated from the EEPROM size on your keyboard's microcontroller. This limit [can be adjusted]({% link manual/advanced/custom-build.md %}#adjusting-feature-limits) at compile time (not the GUI) when building firmware.
